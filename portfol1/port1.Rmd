...
###   Error
performance::icc() error in the last two models,  
I solve this problem by calculating by hand. 
...
(source: #?icc(), R Documentation if icc() in performance package)
...
Due to convergence problem, I will use grand-mean centered level-2 variable. 
...
$$\rho = \frac{\sigma^{2}_{u0}}{\sigma^{2}_{u0} + \sigma^{2}_{e}}$$
$$\rho = \frac{0.286}{0.286 + 0.551}$$
```{r, warning=FALSE, message=FALSE}

#performance::performance(nointer)

# Error message: Random effect variances not available. Returned R2 does not account for random effects.
```
...
$$\rho = \frac{\sigma^{2}_{u0}}{\sigma^{2}_{u0} + \sigma^{2}_{e}}$$
$$\rho = \frac{0.286486}{0.286486 + 0.552080}$$
```{r, warning=FALSE, message=FALSE}
